<!-- Generated: 2026-05-23T03:01:45.429Z | Updated: 2026-05-23T03:01:45.429Z -->
<!-- Parent: ../AGENTS.md -->

# db

## Purpose
This directory contains the db module of the project.

## Key Files
| File | Description |
|------|-------------|
| AGENTS.md | Markdown documentation |
| CLAUDE.md | Markdown documentation |
| drizzle.config.ts | TypeScript source |
| package.json | JSON config/data |
| tsconfig.build.json | JSON config/data |
| tsconfig.json | JSON config/data |
| tsconfig.tsbuildinfo | Project file |


## Subdirectories
| Directory | Description | AGENTS |
|-----------|-------------|--------|
| drizzle/ | Project subdirectory | [AGENTS.md](drizzle/AGENTS.md) |
| src/ | Source code | [AGENTS.md](src/AGENTS.md) |
| tests/ | Test suite | [AGENTS.md](tests/AGENTS.md) |


## For AI Agents


## Dependencies
- **Internal:** drizzle/, src/, tests/
- **External:** See package.json for full dependency list

<!-- MANUAL:START -->
<!-- MANUAL:END -->
