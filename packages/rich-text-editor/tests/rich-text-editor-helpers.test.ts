...
} from "../src/rich-text-editor-helpers";
...
describe("buildRichTextToolbarState", () => {
  it("builds a full toolbar state snapshot from the editor activity map", () => {
    const editor = {
      isActive(
        ...
        if (
          typeof nameOrAttributes === "object" &&
    ...
    expect(toolbar.find((item) => item.key === "alignCenter")?.active).toBe(
      true
    );
  ...
  it("treats left alignment as the default active state", () => {
    ...
    expect(toolbar.find((item) => item.key === "alignRight")?.active).toBe(
      false
    );
  });

  it("returns all disabled items when editor is null", () => {
    const toolbar = buildRichTextToolbarState(null);
...
describe("getRichTextMediaInsertions", () => {
  it("returns image insert payloads for image uploads", () => {
    expect(
      getRichTextMediaInsertions("image", [
        {
          id: "img_1",
      ...
      {
        type: "image",
        attrs: { src: "https://cdn.example.com/cover.jpg", alt: "cover.jpg" },
      },
    ]);
  });

  it("returns videoBlock insert payloads for video uploads", () => {
    expect(
      getRichTextMediaInsertions("video", [
        { id: "vid_1", url: "https://cdn.example.com/clip.mp4" },
      ])
    ).toEqual([
      {
        type: "videoBlock",
  ...
  it("returns empty array for empty assets", () => {
    expect(getRichTextMediaInsertions("image", [])).toEqual([]);
    expect(getRichTextMediaInsertions("video", [])).toEqual([]);
  });
});
...
describe("normalizeRichTextLinkHref", () => {
  it("normalizes bare domains into https urls", () => {
    expect(normalizeRichTextLinkHref("example.com/article")).toBe(
      "https://example.com/article"
    );
    expect(normalizeRichTextLinkHref("www.example.com")).toBe(
      "https://www.example.com"
    );
  });

  it("preserves explicit schemes and clears blank input", () => {
    expect(normalizeRichTextLinkHref("https://example.com")).toBe(
      "https://example.com"
    );
    expect(normalizeRichTextLinkHref("mailto:hello@example.com")).toBe(
