<!-- Generated: 2026-05-23T03:01:45.429Z | Updated: 2026-05-23T03:01:45.429Z -->
<!-- Parent: ../AGENTS.md -->

# messages

## Purpose
This directory contains the messages module of the project.

## Key Files
| File | Description |
|------|-------------|
| admin-message-navigation.ts | TypeScript source |
| admin-messages-page.tsx | React component |
| admin-moderation-todos-page.tsx | React component |
| AGENTS.md | Markdown documentation |
| CLAUDE.md | Markdown documentation |


## Subdirectories
| Directory | Description | AGENTS |
|-----------|-------------|--------|


## For AI Agents


## Dependencies
- **Internal:** None
- **External:** See package.json for full dependency list

<!-- MANUAL:START -->
<!-- MANUAL:END -->
