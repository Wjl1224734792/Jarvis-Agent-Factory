<!-- Generated: 2026-05-23T03:01:45.429Z | Updated: 2026-05-23T03:01:45.429Z -->
<!-- Parent: ../AGENTS.md -->

# admin-reports

## Purpose
This directory contains the admin-reports module of the project.

## Key Files
| File | Description |
|------|-------------|
| admin-reports.helpers.ts | TypeScript source |
| admin-reports.route.ts | TypeScript source |
| admin-reports.service.ts | TypeScript source |
| AGENTS.md | Markdown documentation |
| CLAUDE.md | Markdown documentation |


## Subdirectories
| Directory | Description | AGENTS |
|-----------|-------------|--------|


## For AI Agents


## Dependencies
- **Internal:** None
- **External:** See package.json for full dependency list

<!-- MANUAL:START -->
<!-- MANUAL:END -->
