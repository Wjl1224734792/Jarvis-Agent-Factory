...
单一编排者通过 `spawn` 统一调度子代理，完成从需求澄清到评审交付的 6 阶段流程。
...
| 阶段 | 名称 | 执行方式 | 主要产物 |
|------|------|----------|----------|
| 1 | 需求澄清 | 主会话直接与用户对话，不得 `spawn` | `docs/requirements/` |
...
2. 阶段 1 必须先澄清需求，再进入文档与实现阶段。
3. `spawn` 时必须传递足够的上游上下文，不能让子代理盲做。
4. 高风险共享区域只能有一个明确责任方。
5. 调整共享契约或共享实现时，必须留下明确的计划补丁或文档痕迹。
