...
所有正式产物**必须**存放在 `.jarvis/<YYYY>-<MM>-<DD>/` 日期目录下：
...
> **禁止**旧扁平格式 `.jarvis/requirements/xxx.md` 或 `.jarvis/requirements/YYYY-MM-DD-xxx.md`，引擎已移除所有向后兼容回退逻辑。
...
| `2026-05-19/` | v4.2.0 — /ask 4模式 + /simplify/trace/improve + /auto智能路由 + 评分权重 + 孤立技能全接入 + 平台Agent全覆盖 |
