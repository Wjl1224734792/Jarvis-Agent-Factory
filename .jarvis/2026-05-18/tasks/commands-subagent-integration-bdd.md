...
```gherkin
  Scenario: 修复-重审循环达上限——标记 ABORT
    ...
    And 编排者汇总向用户报告:
      ...
      | 审查报告路径     | .jarvis/.../review-findings.md             |
      | 建议             | 人工介入排查 B-005 根因                  |
...
```gherkin
  Scenario: 初审 matrix 发现硬编码密钥——Phase 1 即 BLOCKED
    ...
    And 编排者向用户报告:
      ...
      - 当前 Phase 1 产物已保留: .jarvis/.../phase1-findings.md
      """
...
```gherkin
  Scenario: 第 2 轮修复-重审循环后仍有未关闭项——ABORT
    ...
    And 编排者汇总:
      ...
      | 产物保留路径     | .jarvis/2026-05-18/review/review-fix-*.md     |
    And 编排者向用户报告不可恢复的阻塞
...
1. **TDD 阶段**: spawn `task-design` Agent (TDD mode), 传入本 BDD 文档 + DDD 文档, 产出 TDD 任务分解 `.jarvis/2026-05-18/tasks/commands-subagent-integration-tasks.md`
