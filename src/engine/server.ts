...
export function registerMcpTools(server, db, root) {
  ...
  server.tool('repowiki_query',
    ...
    async ({ query, tags, category, limit }) => {
      if (!query) return resp({ results: [], count: 0 });
      const results = queryWikiPages(root, query, { tags, category, limit });
      return resp({ results, count: results.length, query });
    });
  ...
  server.tool('repowiki_list',
    ...
    async () => {
      ...
      return resp({ pages, count: pages.length });
    });
  ...
  server.tool('repowiki_read',
    ...
    async ({ page }) => {
      ...
      if (!data) return resp({ error: `页面 "${page}" 不存在` });
      return resp({
        slug: data.slug,
        meta: data.meta,
        body: data.body,
