...
export const GATE_CONFIG: Record<string, GateConfig> = {
  ...
  'Gate E':      { dir: 'shipping',          check: '质量门重检通过（Lint+Type-check+Build+Deps Audit）+测试套件重跑通过+安全审计+上线检查清单+回滚预案就绪',                           operations: { allow: ['read','deploy','write_doc','lint','build','spawn_impl'], deny: ['write_code','spawn_test'] },                                 agent_guide: { can_spawn: ['security-review-expert', 'infra-deploy-expert', 'docs-engineer'], note: '发布阶段——安全审计+文档生成+上线检查+版本管理+归档' },                                                                                                                                                                                  max_retry: 2, entry_condition: 'Gate D 审查通过' },
