...
export const GATE_CONFIG: Record<string, GateConfig> = {
  ...
  'H2': { dir: 'hotfix', check: '快速验证通过（Lint+Type-check+Build+Test重检），修复后功能正常，回滚预案可执行',      operations: { allow: ['read','spawn_test','fix'],                     deny: ['write_code','spawn_impl','build','deploy'] },                agent_guide: { can_spawn: ['frontend-test-expert','backend-test-expert','browser-test-expert','kotlin-test-expert', 'swift-test-expert', 'flutter-test-expert', 'taro-test-expert', 'expo-test-expert'], note: '快速验证+回滚预案' },                                                                                                                                                              max_retry: 2, entry_condition: 'Gate H1 最小化修复已提交' },
  ...
  'E2': { dir: 'evaluation',  check: '评估用例全部运行完毕，指标数据已收集',                                        operations: { allow: ['read','spawn_test'],                            deny: ['write_code','spawn_impl','build','deploy'] },                agent_guide: { can_spawn: ['frontend-test-expert','backend-test-expert','perf-test-expert','kotlin-test-expert', 'swift-test-expert', 'flutter-test-expert', 'taro-test-expert', 'expo-test-expert'], note: '运行用例+收集指标' },                                                                                                                                                                    max_retry: 2, entry_condition: 'Gate E1 原型已生成' },
  ...
  'RL3': { dir: 'shipping',   check: 'CI通过（若有）+Commit+Tag+Push+npm publish已完成',                                    operations: { allow: ['read','deploy','write_doc'],                   deny: ['write_code','spawn_impl','spawn_test','lint','build'] },     agent_guide: { can_spawn: ['infra-deploy-expert', 'security-review-expert'], note: '发布执行——commit+tag+push+npm publish；安全审计并行' },                                                                                                                                                 max_retry: 2, entry_condition: 'Gate RL2 版本已递增' },
