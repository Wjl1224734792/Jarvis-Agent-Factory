...
export const GATE_CONFIG: Record<string, GateConfig> = {
  ...
  'Gate A':      { dir: 'requirements',     check: '至少1个需求文档，含REQ-XXX编号',                                                                                 operations: { allow: ['read','write_doc','spawn_impl'],               deny: ['write_code','spawn_test','build','deploy'] },                                 agent_guide: { can_spawn: ['code-explore-expert', 'external-resource-expert', 'docs-research-expert', 'frontend-debug-expert'], note: '需求澄清——spawn code-explore-expert探索代码库+external-resource-expert外部知识+docs-research-expert文档调研+frontend-debug-expert探索现有UI（按需），纯只读探索不写代码', team_strategy: 'subagent_only' },                             max_retry: Infinity },
