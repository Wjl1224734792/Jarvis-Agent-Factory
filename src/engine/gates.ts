...
export const PIPELINE_DEFS = {
  ...
  /** 自动路由：支持 Gate 入口跳转，按任务类型智能跳过无关闸门 */
  auto: {
    name: '自动路由',
    gates: ['Gate A', 'Gate B-DDD', 'Gate B-BDD', 'Gate B-TDD', 'Gate B1', 'Gate C', 'Gate C-impl', 'Gate C1', 'Gate C1.5', 'Gate C2', 'Gate D', 'Gate R', 'Gate E'],
    allow_jump: true,
  },
  ...
  /** 深度初始化：AI驱动的分层架构文档初始化
   *  Gate 命名：DI0-DI3 属于 /deepinit 流水线 */
  deepinit: {
    name: '深度初始化',
    gates: ['DI0', 'DI1', 'DI2', 'DI3'],
  },
  /** 架构咨询：架构师直接对话咨询，单 Gate C0 对话记录归档 */
  consult: {
    name: '架构咨询',
    gates: ['C0'],
  },
...
/** 默认流水线的 Gate 序列
 * @deprecated 使用 getPipelineGates(type) 代替，支持按流水线类型动态获取 */
...
export const GATE_CONFIG: Record<string, GateConfig> = {
  // ── full / frontend / backend / auto 共用 Gate ──
  'Gate A':      { dir: 'requirements',     check: '至少1个需求文档，含REQ-XXX编号',                                                                                 operations: { allow: ['read','write_doc','spawn_impl'],               deny: ['write_code','spawn_test','build','deploy'] },                                 agent_guide: { can_spawn: ['code-explore-expert', 'external-resource-expert', 'docs-research-expert', 'frontend-debug-expert'], note: '需求澄清——spawn code-explore-expert探索代码库+external-resource-expert外部知识+docs-research-expert文档调研+frontend-debug-expert调试/探索现有UI（按需），纯只读探索不写代码', team_strategy: 'subagent_only' },                             max_retry: Infinity },
  ...
  'Gate B1':     { dir: 'architecture',      check: '架构评审通过，架构方案文档已产出（涉及前端/后端/数据库/算法的领域均有评审文档）',                                                operations: { allow: ['read','write_doc','sweep_arch','spawn_impl'],   deny: ['write_code','spawn_test','build','deploy'] },                                 agent_guide: { can_spawn: ['frontend-architect', 'backend-architect', 'database-architect', 'algorithm-expert', 'code-explore-expert', 'mobile-architect'], note: '架构评审——spawn对应架构师(依变更范围选择前端/后端/数据库/算法)+code-explore-expert辅助探索现有代码库架构，纯只读不写代码', team_strategy: 'subagent_only' }, max_retry: 2, entry_condition: 'Gate B-TDD TDD任务包已产出' },
  'Gate C':      { dir: 'plans',             check: '计划文档含parallel_batches+Execution Packet',                                                                        operations: { allow: ['read','write_doc','sweep_arch','spawn_impl'], deny: ['write_code','spawn_test','build','deploy'] },                                 agent_guide: { can_spawn: ['planner', 'skill-assignment-expert'], note: '执行规划——用subagent(Agent工具)spawn planner+skill-assignment-expert产出parallel_batches和执行计划', team_strategy: 'subagent_only' },                                               max_retry: 2, entry_condition: 'Gate B1 架构评审通过（或确认无需架构评审）' },
  'Gate C-impl': { dir: 'implementation',    check: '所有Batch实现完成，实现Agent已返回结果',                                                                             operations: { allow: ['read','write_code','spawn_impl'],             deny: ['spawn_test','build','deploy'] },                                 agent_guide: { can_spawn: ['frontend-dev-expert', 'frontend-ui-expert', 'frontend-state-expert', 'backend-dev-expert', 'backend-api-expert', 'backend-logic-expert', 'backend-data-expert', 'flutter-dev-expert', 'flutter-ui-expert', 'flutter-state-expert', 'taro-dev-expert', 'taro-ui-expert', 'taro-state-expert', 'expo-dev-expert', 'expo-ui-expert', 'expo-state-expert', 'kotlin-dev-expert', 'kotlin-ui-expert', 'kotlin-state-expert', 'swift-dev-expert', 'swift-ui-expert', 'swift-state-expert', 'miniprogram-dev-expert', 'miniprogram-ui-expert', 'miniprogram-state-expert', 'uni-app-dev-expert', 'uni-app-ui-expert', 'uni-app-state-expert', 'react-dev-expert', 'react-ui-expert', 'react-state-expert', 'vue-dev-expert', 'vue-ui-expert', 'vue-state-expert', 'remediation-expert', 'remediation-planner'], note: '批量实现——推荐 Agent Team(TeamCreate) 并行调度实现Agent(Team模式),轻量任务用subagent(Agent工具)；平台Agent(kotlin/swift/flutter/taro/expo)按需选择；修复回退时spawn remediation-expert或remediation-planner', team_strategy: 'prefer_team', team_rules: '每个Team成员必须独占模块/文件区域,禁止多成员共享同一文件或模块。前端按组件/页面拆分,后端按服务/路由模块拆分,移动端按平台+页面拆分,共享区域由唯一责任人处理' }, max_retry: 3, entry_condition: 'Gate C 执行计划已产出' },
  ...
  'Gate C1.5':   { dir: 'implementation',    check: '页面/组件视觉验证截图证据已附（含多视口/响应式截图+样式检查）',                                                              operations: { allow: ['read','preview','fix','spawn_test','spawn_impl'],  deny: ['build','deploy','write_code'] },                                 agent_guide: { can_spawn: ['browser-test-expert', 'frontend-debug-expert'], note: '视觉验证门——spawn browser-test-expert 执行浏览器截图+响应式多视口检查+样式验证；或 spawn frontend-debug-expert 执行Chrome DevTools深度视觉调试。失败则退回实现Agent补充证据' },                                                                      max_retry: 2, entry_condition: 'Gate C1 质量检查通过' },
  'Gate C2':     { dir: 'testing',           check: 'quality-gates.yml门禁判定通过：单元测试覆盖率/通过率≥阈值、集成/E2E测试通过率≥阈值、Lint/类型错误≤阈值；测试文档用例覆盖完整，API契约验证通过', operations: { allow: ['read','spawn_impl','spawn_test','fix'],                     deny: ['deploy','write_code'] },                                 agent_guide: { can_spawn: ['test-doc-writer', 'frontend-test-expert', 'backend-test-expert', 'kotlin-test-expert', 'swift-test-expert', 'flutter-test-expert', 'taro-test-expert', 'expo-test-expert', 'miniprogram-test-expert', 'uni-app-test-expert', 'react-test-expert', 'vue-test-expert', 'api-test-expert', 'test-executor', 'remediation-expert', 'browser-test-expert', 'frontend-debug-expert', 'api-contract-expert', 'perf-test-expert', 'e2e-test-expert'], note: '测试阶段——推荐 Agent Team 并行跑测试(TeamCreate→各tester并行执行),轻量检查用subagent。平台测试Agent(kotlin/swift/flutter/taro/expo)按需选择。步骤1(Team并行):spawn test-doc-writer+各平台test-expert → 步骤2:spawn test-executor → 步骤3(失败时):spawn remediation-expert(≤2轮) → 步骤4:spawn e2e-test-expert → 步骤5:汇总至.jarvis/testing/', team_strategy: 'prefer_team', team_rules: '各tester按测试类型独占(单元/集成/E2E/性能/安全),test-doc-writer写文档后tester按文档独立执行,互不干扰。平台测试按平台隔离,互不交叉' }, max_retry: 5, entry_condition: 'Gate C1（后端）或 Gate C1+C1.5（前端/移动端）通过' },
  'Gate D':      { dir: 'review',            check: '领域审查+安全审计+性能审计通过；quality-gates.yml门禁判定通过：安全严重漏洞=0、高危漏洞≤阈值、性能回归≤阈值；REQ追踪矩阵完整',           operations: { allow: ['read','review','audit','fix','spawn_impl'],  deny: ['spawn_test','build','deploy','write_code'] },                                 agent_guide: { can_spawn: ['frontend-review-expert', 'backend-review-expert', 'kotlin-review-expert', 'swift-review-expert', 'flutter-review-expert', 'taro-review-expert', 'expo-review-expert', 'miniprogram-review-expert', 'uni-app-review-expert', 'react-review-expert', 'vue-review-expert', 'security-review-expert', 'perf-review-expert', 'qa-review-expert', 'change-review-expert', 'diff-review-expert', 'project-review-expert', 'review-fix-optimize', 'review-only', 'verify-expert', 'code-explore-expert', 'frontend-debug-expert', 'frontend-architect', 'backend-architect', 'database-architect', 'mobile-architect', 'browser-test-expert'], note: '评审阶段——推荐 Agent Team 并行审查(TeamCreate→各reviewer并行),qa-review-expert用subagent综合签核。架构审查(前端/后端/数据库)也参与评审。平台审查Agent(kotlin/swift/flutter/taro/expo)按需选择', team_strategy: 'prefer_team', team_rules: '各reviewer按领域独占(前端/后端/移动端/安全/性能),只读审查不修改文件。qa-review-expert为唯一签核者,汇总各领域findings后综合判定。平台审查按平台隔离,互不交叉' }, max_retry: 2, entry_condition: 'Gate C2 测试通过' },
  // ── /review-only 专用只读审查 Gate ──
  'Gate R':      { dir: 'review',            check: '结构化审查报告已产出，含按严重度分级的findings+证据依据+风险评估',                                                         operations: { allow: ['read','review','audit','spawn_impl'],        deny: ['write_code','write_doc','fix','spawn_test','build','deploy'] },                                 agent_guide: { can_spawn: ['frontend-review-expert', 'backend-review-expert', 'security-review-expert', 'perf-review-expert', 'qa-review-expert', 'diff-review-expert', 'project-review-expert', 'change-review-expert', 'frontend-architect', 'backend-architect', 'database-architect', 'mobile-architect', 'code-explore-expert'], note: '只读审查——spawn各领域审查Agent进行只读审查，禁止任何代码修改或修复。只产出findings报告，不执行fix', team_strategy: 'prefer_team', team_rules: '各reviewer按领域独占(前端/后端/安全/性能)，只读审查不修改文件。qa-review-expert为唯一签核者。严禁spawn remediation-expert/remediation-planner/review-fix-optimize等修复类Agent' }, max_retry: 2 },
  'Gate E':      { dir: 'shipping',          check: '质量门重检通过（Lint+Type-check+Build+Deps Audit）+CI通过（若有）+测试套件重跑通过+安全审计+上线检查清单+回滚预案就绪',                           operations: { allow: ['read','deploy','write_doc','lint','build','spawn_test','spawn_impl'], deny: ['write_code'] },                                 agent_guide: { can_spawn: ['security-review-expert', 'infra-deploy-expert', 'docs-engineer'], note: '发布阶段——安全审计+文档生成+上线检查+版本管理+归档' },                                                                                                                                                                                  max_retry: 2, entry_condition: 'Gate D 审查通过' },

  // ── /consult 流水线 ──
  'C0': { dir: 'consult',      check: '架构咨询对话已记录归档',                                                         operations: { allow: ['read','write_doc','sweep_arch','spawn_impl'],            deny: ['write_code','spawn_test','build','deploy'] }, agent_guide: { can_spawn: ['frontend-architect','backend-architect','algorithm-expert','database-architect','code-explore-expert','external-resource-expert'], note: '架构咨询——spawn对应架构师直接对话，纯只读+文档产出' },                                                                                              max_retry: Infinity },

  // ── /refactor 流水线 ──
  'R1': { dir: 'refactoring',  check: '重构边界与目标文档已产出，含重构范围+不变行为清单+成功标准',                                                              operations: { allow: ['read','write_doc','spawn_impl'],               deny: ['write_code','spawn_test','build','deploy'] },                agent_guide: { can_spawn: ['code-explore-expert'], note: '定义重构边界与目标——先 spawn code-explore-expert 扫描目标代码区域，收集代码结构、依赖关系和复杂度信息，再基于探索结果定义重构边界' },                                                                             max_retry: 2 },
  'R2': { dir: 'refactoring',  check: '现有测试套件全部通过，基线覆盖率报告已产出',                                                                              operations: { allow: ['read','spawn_impl','spawn_test'],                            deny: ['write_code','build','deploy'] },                agent_guide: { can_spawn: ['frontend-test-expert','backend-test-expert','kotlin-test-expert', 'swift-test-expert', 'flutter-test-expert', 'taro-test-expert', 'expo-test-expert'], note: '运行现有测试套件+基线覆盖率', team_strategy: 'subagent_only' },                                                                                                                                                    max_retry: 2, entry_condition: 'Gate R1 重构边界已确认' },
  'R3': { dir: 'refactoring',  check: '重构代码已提交，所有修改在重构边界内，未涉及边界外文件',                                                                  operations: { allow: ['read','write_code','spawn_impl'],              deny: ['spawn_test','build','deploy'] },                              agent_guide: { can_spawn: ['frontend-dev-expert','frontend-ui-expert','frontend-state-expert','backend-dev-expert','backend-api-expert','backend-logic-expert','backend-data-expert','kotlin-dev-expert', 'swift-dev-expert', 'flutter-dev-expert', 'taro-dev-expert', 'expo-dev-expert','remediation-expert'], note: '执行重构——推荐 Agent Team 并行执行,各成员独占模块', team_strategy: 'prefer_team', team_rules: '重构按模块拆分,每个Team成员独占一个模块,共享区域由唯一责任人处理' }, max_retry: 2, entry_condition: 'Gate R2 基线测试通过+覆盖率已记录' },
  'R4': { dir: 'refactoring',  check: '测试套件再次全部通过，覆盖率对比无下降，行为漂移检测通过',                                                                operations: { allow: ['read','spawn_impl','spawn_test'],                            deny: ['write_code','build','deploy'] },                agent_guide: { can_spawn: ['frontend-test-expert','backend-test-expert','kotlin-test-expert', 'swift-test-expert', 'flutter-test-expert', 'taro-test-expert', 'expo-test-expert'], note: '再次运行测试+对比覆盖率+行为漂移检测, 可并行执行前后端测试', team_strategy: 'prefer_team', team_rules: '前端测试和后端测试各自独占,互不干扰,结果汇总对比' },                                                                      max_retry: 2, entry_condition: 'Gate R3 重构代码已提交' },
  ...
  'H1': { dir: 'hotfix', check: '最小化修复代码已提交，修复范围严格限定在故障根因，未夹带无关改动',                  operations: { allow: ['read','write_code','spawn_impl'],              deny: ['spawn_test','build','deploy'] },                              agent_guide: { can_spawn: ['frontend-dev-expert','backend-dev-expert','kotlin-dev-expert', 'swift-dev-expert', 'flutter-dev-expert', 'taro-dev-expert', 'expo-dev-expert','remediation-expert'], note: '最小化修复' },                                                                                                                                                                          max_retry: 2, entry_condition: 'Gate H0 审批人已确认' },
  'H2': { dir: 'hotfix', check: '快速验证通过（Lint+Type-check+Build+Test重检），修复后功能正常，回滚预案可执行',      operations: { allow: ['read','spawn_impl','spawn_test','fix'],                     deny: ['write_code','build','deploy'] },                agent_guide: { can_spawn: ['frontend-test-expert','backend-test-expert','browser-test-expert','kotlin-test-expert', 'swift-test-expert', 'flutter-test-expert', 'taro-test-expert', 'expo-test-expert'], note: '快速验证+回滚预案' },                                                                                                                                                              max_retry: 2, entry_condition: 'Gate H1 最小化修复已提交' },
  'H3': { dir: 'hotfix', check: '事后回溯审计报告已产出，含根因分析+修复措施+预防改进',                            operations: { allow: ['read','review','audit','deploy','write_doc','spawn_impl'],  deny: ['write_code','spawn_test'] },                  agent_guide: { can_spawn: ['security-review-expert','qa-review-expert','docs-engineer'], note: '事后回溯审计' },                                                                                                                                                                        max_retry: Infinity, entry_condition: 'Gate H2 快速验证通过+回滚预案已就绪' },

  // ── /migrate 流水线 ──
  'M1': { dir: 'migration', check: '迁移规则文档已产出，规则覆盖率验证通过',                              operations: { allow: ['read','write_doc','spawn_impl'],                            deny: ['write_code','spawn_test','build','deploy'] }, agent_guide: { can_spawn: ['code-explore-expert'], note: '验证迁移规则覆盖率' },                                                                                                                                                                                                                         max_retry: 2 },
  'M2': { dir: 'migration', check: '迁移已执行，代码已按规则表转换完毕',                                    operations: { allow: ['read','write_code','spawn_impl'],              deny: ['spawn_test','build','deploy'] },                              agent_guide: { can_spawn: ['frontend-dev-expert','backend-dev-expert','kotlin-dev-expert', 'swift-dev-expert', 'flutter-dev-expert', 'taro-dev-expert', 'expo-dev-expert','remediation-expert'], note: '应用迁移' },                                                                                                                                                                          max_retry: 2, entry_condition: 'Gate M1 规则覆盖率验证通过' },
  ...
  'E1': { dir: 'evaluation',  check: '快速原型已生成（沙箱/独立分支），可独立运行',                                   operations: { allow: ['read','write_code','spawn_impl'],              deny: ['spawn_test','build','deploy'] },                              agent_guide: { can_spawn: ['frontend-dev-expert','backend-dev-expert','kotlin-dev-expert', 'swift-dev-expert', 'flutter-dev-expert', 'taro-dev-expert', 'expo-dev-expert'], note: '生成快速原型（沙箱）' },                                                                                                                                                                                          max_retry: 2, entry_condition: 'Gate E0 评估标准已定义' },
  'E2': { dir: 'evaluation',  check: '评估用例全部运行完毕，指标数据已收集',                                        operations: { allow: ['read','spawn_impl','spawn_test'],                            deny: ['write_code','build','deploy'] },                agent_guide: { can_spawn: ['frontend-test-expert','backend-test-expert','perf-test-expert','kotlin-test-expert', 'swift-test-expert', 'flutter-test-expert', 'taro-test-expert', 'expo-test-expert'], note: '运行用例+收集指标' },                                                                                                                                                                    max_retry: 2, entry_condition: 'Gate E1 原型已生成' },
  ...
  // ── /debug 流水线 ──
  'D0': { dir: 'debug',       check: '异常描述已记录，日志/堆栈/环境信息已收集',                                    operations: { allow: ['read','write_doc','spawn_impl'],               deny: ['write_code','spawn_test','build','deploy'] }, agent_guide: { can_spawn: ['code-explore-expert', 'external-resource-expert'], note: '异常描述+日志收集' },                                                                                                                                                                                                                 max_retry: 2 },
  'D1': { dir: 'debug',       check: '最小复现用例已生成，可稳定复现异常',                                          operations: { allow: ['read','write_code','spawn_impl'],              deny: ['spawn_test','build','deploy'] },                              agent_guide: { can_spawn: ['frontend-dev-expert','backend-dev-expert','kotlin-dev-expert', 'swift-dev-expert', 'flutter-dev-expert', 'taro-dev-expert', 'expo-dev-expert', 'remediation-expert'], note: '生成最小复现用例' },                                                                                                                                                                                          max_retry: 2, entry_condition: 'Gate D0 异常信息已收集' },
  'D2': { dir: 'debug',       check: '调试会话已启动，断点/日志已插入关键路径',                                       operations: { allow: ['read','write_code','spawn_impl'],              deny: ['spawn_test','build','deploy'] },                              agent_guide: { can_spawn: ['frontend-dev-expert','backend-dev-expert','kotlin-dev-expert', 'swift-dev-expert', 'flutter-dev-expert', 'taro-dev-expert', 'expo-dev-expert', 'code-explore-expert'], note: '启动调试会话' },                                                                                                                                                                                              max_retry: 2, entry_condition: 'Gate D1 复现用例已生成' },
  'D3': { dir: 'debug',       check: '交互式诊断完成，根因已定位',                                                operations: { allow: ['read','write_code','spawn_impl','spawn_test'], deny: ['build','deploy'] },                                         agent_guide: { can_spawn: ['frontend-dev-expert','backend-dev-expert','kotlin-dev-expert', 'swift-dev-expert', 'flutter-dev-expert', 'taro-dev-expert', 'expo-dev-expert', 'remediation-expert'], note: '交互式诊断' },                                                                                                                                                                                                  max_retry: Infinity, entry_condition: 'Gate D2 调试会话已启动' },
  ...
  'RL3': { dir: 'shipping',   check: 'CI通过（若有）+Commit+Tag+Push+npm publish已完成',                                    operations: { allow: ['read','deploy','write_doc','spawn_impl'],                   deny: ['write_code','spawn_test','lint','build'] },     agent_guide: { can_spawn: ['infra-deploy-expert', 'security-review-expert'], note: '发布执行——commit+tag+push+npm publish；安全审计并行' },                                                                                                                                                 max_retry: 2, entry_condition: 'Gate RL2 版本已递增' },
  ...
  // ── /ask 流水线 ──
  'K0': { dir: 'requirements', check: '需求摄入完成，模式已选择（Interview/Direct/Consensus/Review），核心问题或需求已明确',    operations: { allow: ['read','write_doc','spawn_impl'],               deny: ['write_code','spawn_test','build','deploy'] },             agent_guide: { can_spawn: ['code-explore-expert', 'docs-research-expert', 'external-resource-expert'], note: '需求摄入——先加载会话上下文→快速解析提取主题词→spawn靶向探索Agent(按需派出,最多10个)→基于探索结果进行模式判定和苏格拉底式追问', team_strategy: 'subagent_only' },                                                                             max_retry: Infinity },
  ...
  'S1': { dir: 'simplification', check: '简化执行完成，代码已按分析报告优化，所有功能保持不变',                              operations: { allow: ['read','write_code','spawn_impl'],              deny: ['spawn_test','build','deploy'] },                              agent_guide: { can_spawn: ['frontend-dev-expert', 'backend-dev-expert', 'backend-logic-expert', 'kotlin-dev-expert', 'swift-dev-expert', 'flutter-dev-expert', 'taro-dev-expert', 'expo-dev-expert', 'remediation-expert'], note: '简化执行——推荐 Agent Team 按模块并行简化，各成员独占文件区域，确保功能不变', team_strategy: 'prefer_team', team_rules: '每个Team成员独占模块/文件区域,禁止多成员共享同一文件。前端按组件拆分,后端按服务模块拆分。只做删除冗余+提取复用+简化逻辑,不改变功能行为' }, max_retry: 2, entry_condition: 'Gate S0 代码分析报告已产出' },
  'S2': { dir: 'simplification', check: '回归验证通过：Lint+Type-check+Build+Test全部通过，无回归',                    operations: { allow: ['read','lint','build','spawn_impl','spawn_test','fix'],      deny: ['write_code','deploy'] },                         agent_guide: { can_spawn: ['frontend-test-expert', 'backend-test-expert', 'kotlin-test-expert', 'swift-test-expert', 'flutter-test-expert', 'taro-test-expert', 'expo-test-expert'], note: '回归验证——Lint+Type-check+Build+Test全跑，确保简化无回归', team_strategy: 'subagent_only' },                                                                                                                                      max_retry: 3, entry_condition: 'Gate S1 简化执行已完成' },
  ...
  // ── /trace 流水线 ──
  'T0': { dir: 'trace', check: '问题框架已明确，含症状描述+上下文+已知信息+时间线',                            operations: { allow: ['read','write_doc','spawn_impl'],               deny: ['write_code','spawn_test','build','deploy'] },             agent_guide: { can_spawn: ['code-explore-expert'], note: '问题框架——spawn code-explore-expert收集相关代码上下文和日志模式，编排者定义症状、上下文、已知信息、时间线' },                                                                                                                                    max_retry: 2 },
  ...
  // ── /deepinit 流水线 ──
  'DI0': { dir: 'deepinit',   check: '扫描目标目录已完成，目录结构已生成',                                                operations: { allow: ['read','write_doc','spawn_impl'],               deny: ['write_code','spawn_test','build','deploy'] }, agent_guide: { can_spawn: ['code-explore-expert'], note: '深度初始化——扫描目标目录结构，生成目录树' },                                                                                                                                                                                max_retry: 2 },
  'DI1': { dir: 'deepinit',   check: '初始 AGENTS.md 骨架已生成',                                                      operations: { allow: ['read','write_doc','spawn_impl'],               deny: ['write_code','spawn_test','build','deploy'] }, agent_guide: { can_spawn: ['code-explore-expert'], note: '深度初始化——为根目录生成初始AGENTS.md骨架' },                                                                                                                                                                            max_retry: 2, entry_condition: 'Gate DI0 目录扫描已完成' },
  'DI2': { dir: 'deepinit',   check: '各目录 AGENTS.md 已由AI agent并行读取源码并生成架构文档',                                operations: { allow: ['read','write_doc','spawn_impl'],               deny: ['write_code','spawn_test','build','deploy'] }, agent_guide: { can_spawn: ['code-explore-expert'], note: '深度初始化——spawn AI agent并行读取各目录源码，生成分层的AGENTS.md架构文档', team_strategy: 'prefer_team', team_rules: '每个子目录分配独立Agent，并行读取源码并撰写AGENTS.md，目录间互不交叉' },                                                                      max_retry: 3, entry_condition: 'Gate DI1 根AGENTS.md骨架已生成' },
  'DI3': { dir: 'deepinit',   check: '最终 AGENTS.md 审核完成，含架构图和功能说明',                                             operations: { allow: ['read','write_doc','spawn_impl'],               deny: ['write_code','spawn_test','build','deploy'] }, agent_guide: { can_spawn: ['code-explore-expert'], note: '深度初始化——最终AGENTS.md审核，确认含架构图和功能说明' },                                                                                                                                                          max_retry: 2, entry_condition: 'Gate DI2 各目录AGENTS.md已生成' },

  // ── /improve 流水线 ──
  'IM0': { dir: 'improvement', check: '改进目标已定义，含量化指标+基准值+目标值+停止条件',                         operations: { allow: ['read','write_doc','spawn_impl'],               deny: ['write_code','spawn_test','build','deploy'] },             agent_guide: { can_spawn: ['code-explore-expert', 'external-resource-expert'], note: '目标定义——spawn code-explore-expert+external-resource-expert并行探索，扫描项目技术栈和性能基线；编排者基于探索结果与用户确认改进目标+可量化指标+基准值+目标值+停止条件' },                                                                                            max_retry: 2 },
  ...
  'IM3': { dir: 'improvement', check: '改进已执行，基准测试已运行，结果已记录',                                    operations: { allow: ['read','write_code','spawn_impl','lint','build','spawn_test','fix'], deny: ['deploy'] }, agent_guide: { can_spawn: ['frontend-dev-expert', 'backend-dev-expert', 'backend-logic-expert', 'kotlin-dev-expert', 'swift-dev-expert', 'flutter-dev-expert', 'taro-dev-expert', 'expo-dev-expert', 'remediation-expert', 'frontend-test-expert', 'backend-test-expert', 'kotlin-test-expert', 'swift-test-expert', 'flutter-test-expert', 'taro-test-expert', 'expo-test-expert'], note: '执行验证——推荐 Agent Team 并行实现改进+运行基准测试，各成员独占模块', team_strategy: 'prefer_team', team_rules: '每个Team成员独占模块/文件区域,禁止多成员共享同一文件。实现和测试Agent各自独立,测试确认改进效果' }, max_retry: 3, entry_condition: 'Gate IM2 改进计划已制定' },
...
export const GATE_CHECKS: Record<string, { check: string; max_retry: number }> = Object.fromEntries(
  Object.entries(GATE_CONFIG).map(([k, v]) => [k, { check: v.check, max_retry: v.max_retry }])
);
