...
export const GATE_CONFIG: Record<string, GateConfig> = {
  ...
  'Gate C':      { dir: 'plans',             check: '计划文档含parallel_batches+Execution Packet',                                                                        operations: { allow: ['read','write_doc','sweep_arch','spawn_impl'], deny: ['spawn_test','build','deploy'] },                                 agent_guide: { can_spawn: ['planner', 'skill-assignment-expert'], note: '执行规划——用subagent(Agent工具)spawn planner+skill-assignment-expert产出parallel_batches和执行计划', team_strategy: 'subagent_only' },                                               max_retry: 2, entry_condition: 'Gate B1 架构评审通过（或确认无需架构评审）' },
