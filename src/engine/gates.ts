...
export const GATE_CONFIG: Record<string, GateConfig> = {
  ...
  'Gate C1.5':   { dir: 'implementation',    check: '页面/组件视觉验证截图证据已附（含多视口/响应式截图+样式检查）',                                                              operations: { allow: ['read','preview','fix','spawn_test'],           deny: ['spawn_impl','build','deploy','write_code'] },                                 agent_guide: { can_spawn: ['browser-test-expert', 'frontend-debug-expert'], note: '视觉验证门——spawn browser-test-expert 执行浏览器截图+响应式多视口检查+样式验证；或 spawn frontend-debug-expert 执行Chrome DevTools深度视觉调试。失败则退回实现Agent补充证据' },                                                                      max_retry: 2, entry_condition: 'Gate C1 质量检查通过' },
