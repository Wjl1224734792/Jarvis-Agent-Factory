...
共 **71 个 Agent**，仅 Claude Code 平台。前缀 `oh-my-claudecode:`。详见 `AGENTS.md` L5。
...
共 **40 条命令**，仅 Claude Code 平台。详见 `AGENTS.md` L4。
...
| 命令 | 说明 |
|------|------|
| `/frontend-architect` `/backend-architect` `/algorithm-expert` | 仅方案讨论，不进入流水线 |
...
共 **35 个 Skill**，按 12 类分组。详见 `AGENTS.md` L6。加载方式：`Skill("<skill-name>")`。
...
| `browser-testing` | 浏览器测试方法论（用例/执行/报告/修复闭环） | browser-test-expert |
