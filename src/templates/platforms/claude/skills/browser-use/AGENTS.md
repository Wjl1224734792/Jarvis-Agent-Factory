<!-- Generated: 2026-05-22T07:50:04.735Z | Updated: 2026-05-22T07:50:04.735Z -->
<!-- Parent: ../AGENTS.md -->

# browser-use — Project subdirectory

## Purpose
This directory contains the browser-use module of the project.

## Key Files
| File | Description |
|------|-------------|
| AGENTS.md | Markdown documentation |
| CLAUDE.md | Markdown documentation |
| SKILL.md | Markdown documentation |


## Subdirectories
| Directory | Description | AGENTS |
|-----------|-------------|--------|
| references/ | Project subdirectory | [AGENTS.md](references/AGENTS.md) |


## For AI Agents


## Dependencies
- **Internal:** references/
- **External:** See package.json for full dependency list

<!-- MANUAL:START -->
<!-- MANUAL:END -->
