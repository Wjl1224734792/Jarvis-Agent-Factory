<!-- Generated: 2026-05-22T08:04:04.645Z | Updated: 2026-05-22T08:04:04.645Z -->
<!-- Parent: ../AGENTS.md -->

# writing-skills — Project subdirectory

## Purpose
This directory contains the writing-skills module of the project.

## Key Files
| File | Description |
|------|-------------|
| AGENTS.md | Markdown documentation |
| anthropic-best-practices.md | Markdown documentation |
| CLAUDE.md | Markdown documentation |
| graphviz-conventions.dot | Project file |
| persuasion-principles.md | Markdown documentation |
| render-graphs.js | JavaScript source — No exports |
| SKILL.md | Markdown documentation |
| testing-skills-with-subagents.md | Markdown documentation |


## Subdirectories
| Directory | Description | AGENTS |
|-----------|-------------|--------|
| examples/ | Project subdirectory | [AGENTS.md](examples/AGENTS.md) |


## For AI Agents


## Dependencies
- **Internal:** examples/
- **External:** See package.json for full dependency list

<!-- MANUAL:START -->
<!-- MANUAL:END -->
