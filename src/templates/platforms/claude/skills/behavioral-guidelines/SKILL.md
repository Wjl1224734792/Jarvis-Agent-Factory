...
在执行任何任务前，必须读取以下规则文件（按优先级）：
1. `AGENTS.md` — 项目级 Agent 规范
2. `CLAUDE.md` — 项目级 Claude 指令
3. `.{platform}/rules/*.md` — 平台级编码规范（如 `.claude/rules/*.md`、`.opencode/rules/*.md`、`.codex/rules/*.md`）
4. `.claude/rules/*.md` — Claude Code 通用规则

规则文件包含本项目/平台的编码风格、架构约定、命名规范、安全约束等。**未能读取规则就编码，等同于凭记忆写代码——违反准则 1。**
...
读取规则后，将其约束融入思考过程和实现方案。若规则与任务需求冲突，应明确指出规则来源并征求用户决策，而非默默绕过。
