...
description: "Taro 测试专项工作者：在编排者 分配明确子任务后执行；负责 Taro 小程序/H5 单元测试(Jest)、组件测试和端到端测试(miniprogram-automator)的编写与运行；遵循 TDD Red→Green→Refactor 流程（当 test_strategy 为 tdd 时）。"
