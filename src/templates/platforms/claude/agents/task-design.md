...
- **并行提示**：标注此任务是否可与其他任务共享同类型 Agent 并行（如"与 TASK-002 同为 user 模块 API，共享 Schema → 不可并行"或"独立模块，无共享文件 → 可与同类 Agent 并行"）
...
| "这些任务之间没依赖，全都可以并行" | 无共享依赖的才能并行。两个任务改同一个文件 = 必须串行。但注意：同类型 Agent 不是串行的理由——3 个独立 API 模块可并行 spawn 3 个 backend-api-expert。 |
