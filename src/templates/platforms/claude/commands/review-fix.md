...
model: inherit
argument-hint: [审查范围]
tools: ["Read", "Glob", "Grep", "Bash", "WebFetch", "WebSearch", "Skill", "AskUserQuestion", "Agent", "TeamCreate", "TeamDelete", "SendMessage", "Edit", "Write", "mcp__jarvis-engine__session_join", "mcp__jarvis-engine__gate_jump", "mcp__jarvis-engine__pipeline_guide", "mcp__jarvis-engine__gate_check", "mcp__jarvis-engine__advance_gate", "mcp__jarvis-engine__gate_enforce", "mcp__jarvis-engine__report_status", "mcp__playwright__browser_navigate", "mcp__playwright__browser_snapshot", "mcp__playwright__browser_take_screenshot"]
---
...
3. 确认进入**审查修复优化闭环模式**。完整链路不可跳过、不可倒置（不可绕过）。
   ...
   - 验证全部通过后调用 `mcp__jarvis-engine__gate_enforce()` 确认当前 Gate 条件满足

   ### **阶段五：复审**（不可绕过）
   - 逐项关闭初审 findings，输出关闭矩阵，报告未关闭风险项
   - 可调用 `change-review-expert` Agent
   - 复审通过且关闭矩阵完整后调用 `mcp__jarvis-engine__advance_gate({ gate: "Gate E" })` 推进到发布 Gate
