...
> **自主探索模式**（browser-use-expert 自由探索 → 自动发现 Bug → 出报告）。
> 若需按预先编写的测试用例逐条执行验证，请使用 `browser-test-expert`（结构化用例模式）。
...
- 加入后调用 `mcp__jarvis-engine__gate_jump({ gate: "Gate C2" })` 进入测试验证阶段
...
## 步骤 2：自由探索测试（browser-use-expert）
...
| 模式 | 适用场景 | Agent |
|------|---------|-------|
| **自由探索**（本命令） | 新页面验收、回归探索、未知 Bug 发现 | `browser-use-expert` |
| **结构化用例** | 有明确测试用例、需求验收、CI 回归 | `browser-test-expert` |

两者互补：自由探索发现未知问题 → 结构化用例固化回归 → CI 自动化。
...
`browser-use-expert` 返回后，将其发现汇总到 `docs/testing/YYYY-MM-DD-<topic>-exploration-report.md`：
...
3. 若立即修复：提交给 `/bug-fix` 闭环或 `/review-fix` 闭环
4. 修复后重新探索验证（仅验证修复项）
5. 全部 P0/P1 关闭后 → 闭环完成
