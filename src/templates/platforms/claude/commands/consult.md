...
tools: ["Read", "Bash", "Glob", "Grep", "Skill", "AskUserQuestion", "Agent", "mcp__jarvis-engine__session_join", "mcp__jarvis-engine__pipeline_guide", "mcp__jarvis-engine__gate_check", "mcp__jarvis-engine__advance_gate", "mcp__jarvis-engine__gate_enforce", "WebFetch", "WebSearch", "mcp__jarvis-engine__session_context", "mcp__jarvis-engine__jarvis_priority_context"]
---
...
2. 注册引擎会话：
   - `mcp__jarvis-engine__session_join({ platform: "claude", pipeline_type: "consult" })`
   - 启动对话时调用 `mcp__jarvis-engine__pipeline_guide()` 获取当前工作上下文
...
5. 确认问题后，**必须调用 `Agent` 工具** spawn 对应专家：
   ```
   Agent(
     description="{领域}方案设计",
...
对话完成后：
- `mcp__jarvis-engine__gate_enforce` — 验证咨询完成条件
