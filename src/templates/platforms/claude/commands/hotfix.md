...
tools: ["Read", "Glob", "Grep", "Bash", "Write", "Edit", "Skill", "AskUserQuestion", "Agent", "mcp__jarvis-engine__session_join", "mcp__jarvis-engine__pipeline_guide", "mcp__jarvis-engine__gate_check", "mcp__jarvis-engine__advance_gate", "mcp__jarvis-engine__gate_enforce", "mcp__jarvis-engine__report_status", "WebFetch", "WebSearch", "mcp__jarvis-engine__session_context", "mcp__jarvis-engine__jarvis_priority_context"]
---
...
### 步骤 0：快速信息收集（同一消息同时发出）
Agent(code-explore-expert, "紧急扫描故障相关代码区域：定位堆栈跟踪指向的文件、分析最近git变更、检查相关日志和错误处理路径")

### 步骤 1：定位根因

（参考 debugging-and-error-recovery 技能）：
   - 二分法定位引入故障的 commit
   - 追踪调用链找到故障点
   - 确认根因，不满足于表象修复
