...
> ⚡ 此命令用于算法方案讨论与技术咨询。流水线中算法专家参与两个 Gate：Gate B1（架构设计评审——算法选型与方案评估）和 Gate D（实现审查——算法正确性与复杂度审计，条件性触发）。
...
## 红线
- 只输出方案不修改代码——算法咨询是只读模式
- 复杂度分析必须完整——时间+空间复杂度不可遗漏
- 算法选型必须有对比——至少 2 个备选方案对比
- 不推荐未经验证的算法——仅推荐有理论和工程验证的方案
