...
- **并行探索**: spawn `code-explore-expert` + `external-resource-expert`（spawn 前 `gate_check("read")`）
  - `code-explore-expert`：项目结构、已有 Vue 页面/组件、路由配置、状态管理模式
  - `external-resource-expert`：Vue 3 官方文档、Composition API 最佳实践、相关库文档
...
2. **布局问题**（溢出/重叠/错位）→ 诊断根因（Scoped CSS/V-bind in style/深层选择器问题），修复源文件，重新截图验证
