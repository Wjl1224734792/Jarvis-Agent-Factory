---
description: Expo 开发生命周期——需求→任务→计划→实现→质量→测试→评审→发布完整链路
name: expo
model: inherit
argument-hint: "[Expo需求描述]"
...
| **技术栈** | TypeScript + React Native + Expo SDK 跨端移动端开发 |
| **Gate 序列** | A → B-DDD → B-BDD → B-TDD → B1 → C → C-impl → C1 → C1.5 → C2 → D → E（12 道闸门） |
| **强制 Gate C1.5** | 所有 Expo UI 变更必须过视觉验证（Expo Go 截图） |
...
Gate C-impl:
  Batch 1: [expo-ui-expert, expo-state-expert]   ← UI + 状态可并行
  Batch 2: [expo-dev-expert]                      ← 集成组装
  Batch 3: [expo-test-expert]                     ← 单元/组件测试
```
...
**关键差异**：Gate C1.5（视觉验证）强制不可跳过——所有 Expo UI 变更必须提供 Expo Go 截图证据。
...
- **Gate A 通过后可并行探索**（spawn 前 `gate_check("read")`）：
  ...
  - `external-resource-expert` × N（Expo/React Native 最新文档与最佳实践并行搜索）
...
4. 每个任务 → `Agent()` 调用，选择 Expo 代理路由表中的 `subagent_type`
5. **同 Batch 任务在同一条消息中批量发出**（不可逐个串行）
6. 等待整批完成，检查 plan patch

**Batch 执行顺序**：
- Batch 1: `expo-ui-expert` + `expo-state-expert` 并行
- Batch 2: `expo-dev-expert` 集成组装
- Batch 3: `expo-test-expert` 单元/组件测试
...
**Expo 移动端任务必须过此门。** 条件：
- Expo Go 客户端或 Expo Web 预览已启动
- 修改前/后对比截图已附（使用 Playwright MCP：`browser_snapshot` / `browser_take_screenshot`）
- 移动端视口截图已附（375x812 标准手机视口）
- 关键组件布局和样式已验证
- 无可见布局问题（溢出/重叠/错位）
...
全部实现 Batch 完成
  → 先过 Gate C1.5（视觉验证）
  → spawn expo-test-expert（spawn 前 gate_check("spawn_test")）
  → 执行 Jest + React Native Testing Library 单元/组件测试
  → 可选：spawn browser-test-expert（Web 端交互测试）
  → 可选：spawn e2e-test-expert（Detox E2E 测试）
...
├── spawn expo-review-expert（Expo 代码审查：组件/样式/状态/性能/可访问性）
...
1. 加载 `Skill("code-quality-gate")`，重跑 `npx eslint` + `npx tsc --noEmit` + `npx expo build`
2. 重跑测试套件（Jest + Detox），确保无回归
...
2. 按审查领域并行：expo-review-expert + mobile-architect + security-review-expert
3. 每个审查者独立评审，产出分级报告
...
| Gate C2 | `expo-test-expert` + `browser-test-expert` 可并行；E2E 测试最后执行 |
| Gate D | `expo-review-expert` + `mobile-architect` + `security-review-expert` 并行；完成后 `qa-review-expert` 综合签核 |
...
## 红线
- Expo 任务必须过视觉验证——Gate C1.5 不可跳过
- 组件状态管理必须有对应测试——Gate C2 覆盖率门禁
- UI 组件必须考虑跨端适配——Expo 同时面向 iOS/Android/Web
- 不修改后端 API 契约——Expo 层只管界面和状态，后端接口不可动
- 不修改原生模块配置（app.json / metro.config.js）除非任务明确要求
