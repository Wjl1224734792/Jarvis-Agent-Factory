...
- `code-explore-expert`：项目全景——技术栈、目录结构、已有页面/组件、路由配置、状态管理模式、原生模块配置
- `external-resource-expert`：Expo SDK 最新版本、React Native 更新、跨端最佳实践
...
spawn `code-explore-expert` + `external-resource-expert`（spawn 前 `gate_check("read")`）：
- `code-explore-expert`：需求涉及的特定页面/组件、相关代码路径、依赖链路、Expo 模块依赖
- `external-resource-expert`：需求相关的 expo-* 包文档、iOS/Android/Web 三端兼容性、技术方案参考
