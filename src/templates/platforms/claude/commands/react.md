---
description: React 开发生命周期——需求→任务→计划→实现→质量→测试→评审→发布完整链路（自包含，不依赖 /frontend）
name: react
model: inherit
argument-hint: "[React需求描述]"
...
> React 18/19 + TypeScript Web 开发。架构师: `frontend-architect`。
...
| **强制 Gate C1.5** | 所有 React 任务必须过浏览器视觉验证（Playwright 三视口截图） |
...
| 任务分解 | `task-design` |
| 全栈实现 | `react-dev-expert` |
| UI/布局/样式 | `react-ui-expert` |
| 状态/数据/路由 | `react-state-expert` |
| 测试 | `react-test-expert` |
| 浏览器测试 | `browser-test-expert` |
| 审查 | `react-review-expert` |
...
Gate C-impl:
  Batch 1: [react-ui-expert, react-state-expert]   ← UI + 状态可并行
  Batch 2: [react-dev-expert]                       ← 集成组装（页面/路由/数据流）
  Batch 3: [react-test-expert]                      ← 单元/组件测试
...
- **并行探索**: spawn `code-explore-expert` + `external-resource-expert`（spawn 前 `gate_check("read")`）
  - `code-explore-expert`：项目结构、已有 React 页面/组件、路由配置、状态管理模式
  - `external-resource-expert`：React 18/19 新特性（Server Components/Actions）、相关库文档
- `gate_enforce()` → `advance_gate({ gate: "Gate B-DDD" })`

### Gate B-DDD → B-BDD → B-TDD：任务分解

- spawn `task-design` Agent（DDD→BDD→TDD 三级递进）
  - B-DDD：识别聚合根、实体、值对象、领域服务（React Web 场景的领域建模）
...
- 评审要点：React 组件树分层（Page/Container/Presentational）、路由设计（React Router）、状态管理选型（Context/Zustand/Redux）、数据流设计、SSR/CSR 策略、性能优化策略（memo/useMemo/lazy/Suspense）
...
4. 按 Batch 顺序执行：
   - **Batch 1**：同时 spawn `react-ui-expert` + `react-state-expert`（UI 与状态无依赖，可并行）
   - **Batch 2**：spawn `react-dev-expert`（集成组装：页面组件/路由配置/数据流连接）
   - **Batch 3**：spawn `react-test-expert`（单元测试 Jest + RTL + 组件测试）
...
**React Web 任务必须过浏览器视觉验证。** 条件：
- 开发服务器已启动，通过 Playwright MCP 连接浏览器
- 通过 `mcp__playwright__browser_take_screenshot` 截取三视口截图：
  - **mobile**: 375x812（iPhone 尺寸）
  - **tablet**: 768x1024（iPad 尺寸）
  - **desktop**: 1280x800（标准桌面）
- 通过 `mcp__playwright__browser_snapshot` 获取页面可访问性快照，验证 DOM 结构
- 关键样式属性已验证（通过浏览器 DevTools 检查元素样式）
- 无可见布局问题（溢出/重叠/错位）

**通过**：进入 Gate C2

**不通过**：
1. **证据缺失** → 退回实现 Agent 补充三视口截图和快照
2. **布局问题**（溢出/重叠/错位）→ 诊断根因（CSS 媒体查询/CSS-in-JS/布局组件问题），修复源文件，重新截图验证
...
全部实现 Batch 完成
  ...
  → [可并行] spawn react-test-expert + browser-test-expert（spawn 前 gate_check("spawn_test")）
  → react-test-expert：单元测试（Jest + RTL）+ 组件测试
...
├── spawn react-review-expert（React 代码审查：组件设计/Hooks使用/状态管理/性能/可访问性）
├── spawn frontend-architect（架构一致性审查）
├── spawn security-review-expert（安全审计：XSS/CSP/依赖 CVE/认证授权）
└── spawn perf-review-expert（性能审计：bundle大小/LCP/CLS/FID/内存泄漏）
...
| Gate C2 | `react-test-expert` + `browser-test-expert` 可并行 |
| Gate D | `react-review-expert` + `frontend-architect` + `security-review-expert` + `perf-review-expert` 并行；完成后 `qa-review-expert` 综合签核 |
...
- React 任务必须过浏览器视觉验证——Gate C1.5 不可跳过（三视口 Playwright 截图）
- 组件状态管理必须有对应测试——Gate C2 覆盖率门禁
- UI 组件必须考虑响应式设计——mobile/tablet/desktop 三视口适配是强制要求
- Hooks 使用必须遵守 React Hooks 规则——不在条件/循环中调用，依赖数组完整
- 不修改后端 API 契约——React 前端只管界面和状态，后端接口不可动
- 生产构建无 console 警告/错误——Clean Build 是 Gate C1 门禁

向用户确认已进入 React 开发生命周期模式。
