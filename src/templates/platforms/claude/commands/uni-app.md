---
description: uni-app 开发生命周期——需求→任务→计划→实现→质量→测试→评审→发布完整链路（自包含，不依赖 /frontend）
name: uni-app
model: inherit
argument-hint: "[uni-app需求描述]"
...
> Vue 3 + uni-app 跨端开发（小程序/H5/App）。架构师: `frontend-architect`。
...
| **框架** | uni-app（Vue 3 语法，编译到小程序/H5/App） |
| **Gate 序列** | A → B-DDD → B-BDD → B-TDD → B1 → C → C-impl → C1 → C1.5 → C2 → D → E（12 道闸门） |
| **强制 Gate C1.5** | 所有 uni-app 任务必须过多端视觉验证（微信开发者工具 + H5 浏览器截图） |
...
| 任务分解 | `task-design` |
| 全栈实现 | `uni-app-dev-expert` |
| UI/布局/样式 | `uni-app-ui-expert` |
| 状态/数据/缓存 | `uni-app-state-expert` |
| 测试 | `uni-app-test-expert` |
| 审查 | `uni-app-review-expert` |
...
| Build | `npm run build` |
| Unit Test | Jest |
| E2E Test | @dcloudio/uni-automator |
...
Gate C-impl:
  Batch 1: [uni-app-ui-expert, uni-app-state-expert]   ← UI + 状态可并行
  Batch 2: [uni-app-dev-expert]                         ← 集成组装（页面/路由/跨端适配/条件编译）
  Batch 3: [uni-app-test-expert]                        ← 单元/组件测试
```
...
- **并行探索**: spawn `code-explore-expert` + `external-resource-expert`（spawn 前 `gate_check("read")`）
  - `code-explore-expert`：项目结构、已有 uni-app 页面/组件、pages.json 配置、平台适配代码
  - `external-resource-expert`：uni-app 官方文档、API 变更、跨端兼容性注意事项
- `gate_enforce()` → `advance_gate({ gate: "Gate B-DDD" })`

### Gate B-DDD → B-BDD → B-TDD：任务分解

- spawn `task-design` Agent（DDD→BDD→TDD 三级递进）
  - B-DDD：识别聚合根、实体、值对象、领域服务（uni-app 跨端场景的领域建模）
  - B-BDD：编写 Gherkin 行为场景（覆盖小程序/H5/App 多端）
...
- 评审要点：uni-app 页面/组件分层、pages.json 路由配置与分包、条件编译策略（`#ifdef MP-WEIXIN` / `#ifdef H5` / `#ifdef APP-PLUS`）、状态管理选型（Pinia/Vuex）、跨端 API 兼容性
...
4. 按 Batch 顺序执行：
   - **Batch 1**：同时 spawn `uni-app-ui-expert` + `uni-app-state-expert`（UI 与状态无依赖，可并行）
   - **Batch 2**：spawn `uni-app-dev-expert`（集成组装：页面/路由/跨端适配/条件编译/API 封装）
   - **Batch 3**：spawn `uni-app-test-expert`（单元测试 + 组件测试）
...
**uni-app 跨端任务必须过多端视觉验证。** 条件：
- **微信小程序端**：通过微信开发者工具或 @dcloudio/uni-automator 截取页面截图
- **H5 端**：在浏览器中启动 H5 预览，截取页面截图
- 两端截图对比，确认无平台特异性布局问题
- 关键样式属性已验证（条件编译样式隔离、rpx/upx 单位适配）
- 无可见布局溢出/重叠/错位
...
2. **布局问题**（溢出/重叠/错位）→ 诊断根因（通常是条件编译样式冲突或单位计算问题），修复源文件，重新截图验证
...
全部实现 Batch 完成
  → 先过 Gate C1.5（多端视觉验证）
  → spawn uni-app-test-expert（spawn 前 gate_check("spawn_test")）
  → 单元测试（Jest）+ E2E 测试（@dcloudio/uni-automator）
...
├── spawn uni-app-review-expert（uni-app 代码审查：组件/样式/状态/条件编译/跨端兼容性）
├── spawn frontend-architect（架构一致性审查）
└── spawn security-review-expert（安全审计：XSS/依赖 CVE/小程序安全/App 安全）
...
| Gate C2 | `uni-app-test-expert`（单元 + E2E 串行：单元先，E2E 后） |
| Gate D | `uni-app-review-expert` + `frontend-architect` + `security-review-expert` 并行；完成后 `qa-review-expert` 综合签核 |
...
- UI 组件必须考虑多端适配——条件编译（`#ifdef` / `#ifndef`）是强制要求
- 不修改后端 API 契约——uni-app 前端只管界面和状态，后端接口不可动
- 小程序分包大小不超过 2MB（主包）+ 各分包 2MB——分包策略在架构评审中确认
- 跨端 API 使用 uni-app 统一 API（`uni.xxx`），避免直接调用平台原生 API

向用户确认已进入 uni-app 开发生命周期模式。
