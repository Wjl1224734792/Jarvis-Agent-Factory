---
description: Kotlin 开发生命周期——需求→任务→计划→实现→质量→测试→评审→发布完整链路
name: kotlin
model: inherit
argument-hint: "[Kotlin需求描述]"
tools: ["Read", "Write", "Edit", "Bash", "Glob", "Grep", "Skill", "Agent", "TeamCreate", "SendMessage", "TeamDelete", "mcp__jarvis-engine__session_join", "mcp__jarvis-engine__pipeline_guide", "mcp__jarvis-engine__gate_check", "mcp__jarvis-engine__advance_gate", "mcp__jarvis-engine__gate_enforce", "mcp__jarvis-engine__file_claim_check", "mcp__jarvis-engine__file_claim_register", "mcp__jarvis-engine__file_claim_release"]
---
...
| **技术栈** | Kotlin + Jetpack Compose Android 原生开发 |
| **Gate 序列** | A → B-DDD → B-BDD → B-TDD → B1 → C → C-impl → C1 → C1.5 → C2 → D → E（12 道闸门） |
| **强制 Gate C1.5** | 所有 Compose UI 变更必须过视觉验证（Android Emulator 截图） |

### 可用代理路由

| 层级 | subagent_type |
|------|--------------|
| 架构设计 | `mobile-architect` |
| 任务分解 | `task-design` |
| 全栈实现 | `kotlin-dev-expert` |
| UI/布局/样式 | `kotlin-ui-expert` |
| 状态/数据/路由 | `kotlin-state-expert` |
| 测试 | `kotlin-test-expert` |
| 审查 | `kotlin-review-expert` |
...
| UI Test | Compose Test Rule + Espresso |
...
Gate C-impl:
  Batch 1: [kotlin-ui-expert, kotlin-state-expert]   ← UI + 状态可并行
  Batch 2: [kotlin-dev-expert]                        ← 集成组装
  Batch 3: [kotlin-test-expert]                       ← 单元/组件测试
```
...
**关键差异**：Gate C1.5（视觉验证）强制不可跳过——所有 Compose UI 变更必须提供 Android Emulator 截图证据。
...
- **Gate A 通过后可并行探索**（spawn 前 `gate_check("read")`）：
  ...
  - `external-resource-expert` × N（Kotlin/Compose 最新文档与最佳实践并行搜索）

### Gate B-DDD / B-BDD / B-TDD：任务分解

- `task-design` Agent 按 DDD→BDD→TDD 三级递进顺序执行（不可并行但可连续）
- 产出到 `.jarvis/YYYY-MM-DD/tasks/`

### Gate B1：架构评审

- spawn `mobile-architect`（条件性触发，评审架构方案，只读）
...
4. 每个任务 → `Agent()` 调用，选择 Kotlin 代理路由表中的 `subagent_type`
5. **同 Batch 任务在同一条消息中批量发出**（不可逐个串行）
6. 等待整批完成，检查 plan patch

**Batch 执行顺序**：
- Batch 1: `kotlin-ui-expert` + `kotlin-state-expert` 并行
- Batch 2: `kotlin-dev-expert` 集成组装
- Batch 3: `kotlin-test-expert` 单元/组件测试
...
- 调用 `gate_check` 进入质量检查
- 执行 Kotlin/Android 质量检查命令：
  - Lint：`./gradlew lint`
  - Type-check：`./gradlew compileDebugKotlin`
  - Build：`./gradlew assembleDebug`
  - 三项可并行启动
- 全部通过后调用 `gate_enforce` → `advance_gate`

**质量门不通过**：定位问题文件 → 修复源文件 → 重新过 Gate C1，最多 2 轮；仍不通过 → 标记 `BLOCKED`

### Gate C1.5：视觉验证（强制，不可跳过）

**Kotlin Android 任务必须过此门。** 条件：
- Android Emulator 已启动
- 修改前/后对比截图已附
- 标准手机尺寸截图已附（Pixel 7 412x915 或类似标准尺寸）
- 关键 Composable 布局和样式已验证
- 无可见布局问题（溢出/重叠/错位）
...
全部实现 Batch 完成
  → 先过 Gate C1.5（视觉验证）
  → spawn kotlin-test-expert（spawn 前 gate_check("spawn_test")）
  → 执行 JUnit5 + MockK 单元测试 + Compose Test Rule + Espresso UI 测试
  → 汇总到 .jarvis/YYYY-MM-DD/testing/ → Gate C2 通过
```

**测试失败回退**：
1. 测试失败 → 分析失败报告，定位需修复的实现 Agent
2. spawn 原 Kotlin 实现 Agent 执行修复（传递测试失败报告），修复后重新跑对应测试
3. 最多 2 轮修复-重测循环
...
├── spawn kotlin-review-expert（Kotlin 代码审查：Composable/状态/性能/可访问性）
├── spawn mobile-architect（架构一致性验证）
└── spawn security-review-expert（安全审计：依赖 CVE/Keystore 存储/安全传输）
...
1. 加载 `Skill("code-quality-gate")`，重跑 `./gradlew lint` + `./gradlew compileDebugKotlin` + `./gradlew assembleDebug`
2. 重跑测试套件（JUnit5 + MockK + Compose Test Rule + Espresso），确保无回归
...
- 加载 `Skill("shipping-and-launch")` 执行上线检查清单（含 Google Play 提审清单）
- 加载 `Skill("git-workflow-and-versioning")` 更新版本与 changelog
- 加载 `Skill("finishing-a-development-branch")` 归档
...
2. 按测试类型并行：单元测试 Agent + UI 测试 Agent
...
2. 按审查领域并行：kotlin-review-expert + mobile-architect + security-review-expert
3. 每个审查者独立评审，产出分级报告
...
| Gate C1 | Lint + Type-check + Build 三项可并行启动 |
| Gate C2 | `kotlin-test-expert` 执行单元+UI 测试 |
| Gate D | `kotlin-review-expert` + `mobile-architect` + `security-review-expert` 并行；完成后 `qa-review-expert` 综合签核 |

---

## 故障恢复

Agent 失败重试（最多 3 次）→ Batch 部分失败仅重试失败任务 → Gate 失败回退修复 → 会话检查点支持中断恢复。

---

## 红线
- Kotlin 任务必须过视觉验证——Gate C1.5 不可跳过
- Composable 状态管理必须有对应测试——Gate C2 覆盖率门禁
- UI Composable 必须考虑不同屏幕尺寸/密度适配——响应式布局是强制要求
- 不修改后端 API 契约——Kotlin 层只管界面和状态，后端接口不可动
- 不修改 AndroidManifest.xml/build.gradle 等应用级配置除非任务明确要求
