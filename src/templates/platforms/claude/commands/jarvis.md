...
### 步骤 3：澄清后靶向探索（需求确认后，同一消息同时发出）

需求文档产出后，spawn `code-explore-expert` + `external-resource-expert` + `docs-research-expert`（spawn 前 `gate_check("read")`）：
