...
argument-hint: "[任务描述]"
model: inherit
tools: ["Read", "Write", "Edit", "Bash", "Glob", "Grep", "Skill", "Agent", "WebFetch", "WebSearch", "TeamCreate", "SendMessage", "TeamDelete", "mcp__jarvis-engine__session_join", "mcp__jarvis-engine__pipeline_guide", "mcp__jarvis-engine__gate_check", "mcp__jarvis-engine__advance_gate", "mcp__jarvis-engine__gate_enforce", "mcp__jarvis-engine__file_claim_check", "mcp__jarvis-engine__file_claim_register", "mcp__jarvis-engine__file_claim_release", "mcp__jarvis-engine__session_context", "mcp__jarvis-engine__jarvis_priority_context"]
---
...
以下约束通过 `.claude/settings.json` 的 `PreToolUse` hooks 在代码层面强制执行，**不是建议，是硬性阻断**：
...
### 步骤 1：并行信息收集（需求澄清前，同一消息同时发出，最多 10 个）

在写需求文档之前，先并行收集项目上下文：

Agent(code-explore-expert, "探索项目整体结构、技术栈、核心模块，输出项目全景图")
Agent(docs-research-expert, "调研 AGENTS.md / README.md / CHANGELOG.md，确认当前架构基线")  
Agent(external-resource-expert, "搜索相关技术的最新版本、API变更和最佳实践")

探索结果回来后，整理为"项目上下文摘要"，用于后续需求澄清和文档撰写。
...
**与用户确认需求后**，产出需求文档到 `.jarvis/YYYY-MM-DD/requirements/<topic>.md`。

### 步骤 3：澄清后靶向探索（需求确认后，同一消息同时发出，最多 10 个）

需求文档产出后，spawn `code-explore-expert` + `external-resource-expert` + `docs-research-expert`（最多 10 个，spawn 前 `gate_check("read")`）：
- `code-explore-expert`：需求涉及的特定模块、相关代码路径、依赖链路、可复用组件
- `external-resource-expert`：需求相关的 API 文档、库版本兼容性、技术方案参考
- `docs-research-expert`：项目内相关 ADR/架构决策、已有设计文档

探索结果整理为"靶向上下文摘要"，注入后续 Gate B 任务分解和 Gate C 实现规划。
...
**引擎验证**：计划文档 + Skill 清单产出后，调用 `gate_enforce` → `advance_gate({ gate: "Gate C-impl" })` 自动推进。
...
| 规划 | `planner` |
| 验证 | `verify-expert` |
| 技能分配 | `skill-assignment-expert` |
| 调试 | `frontend-debug-expert` |
| 探索 | `code-explore-expert` / `external-resource-expert` / `docs-research-expert` |
...
**触发条件**：涉及前端页面/组件变更 → **直接执行，不询问用户**。纯后端/逻辑/算法任务跳过。
