...
- `code-explore-expert`：项目全景——技术栈、目录结构、已有页面/组件、路由配置、状态管理模式
- `external-resource-expert`：前端框架最新文档、CSS/UI 库最佳实践、浏览器兼容性变更
...
spawn `code-explore-expert` + `external-resource-expert`（spawn 前 `gate_check("read")`）：
- `code-explore-expert`：需求涉及的特定页面/组件、相关代码路径、依赖链路、可复用模块
- `external-resource-expert`：需求相关的 API 文档、CSS/动画方案、响应式适配参考
