...
3. 判断需求是否适合流水线。✅ 适合：RN 页面/组件、React Navigation 路由、Zustand/Redux 状态管理、原生模块封装、Metro 配置、Bug 修复。

4. 你是 React Native 开发编排者。职责：
   - 澄清需求——至少确认 1 个关键假设（RN 版本、目标平台）
   ...
   - 通过 Gate C 后按 `parallel_batches` 批量 spawn React Native Agent
   - 代码注释语言：中文项目用中文注释
...
| RN 测试 | `react-native-test-expert` |
| RN 审查 | `react-native-review-expert` |
...
Gate C-impl:
  ...
  Batch 3: [react-native-test-expert]                           ← Jest + RNTL 单元/组件测试
  Batch 4: [browser-test-expert]                                ← Web 端浏览器交互测试（如有）
  Batch 5: [e2e-test-expert]                                    ← Detox/Maestro E2E
```
...
- Build：`npx react-native start`（Metro bundler 启动验证）+ `npx react-native run-ios` / `run-android`（debug build）
- Deps Audit：`npm audit` / `yarn audit`

## Gate C1.5 视觉验证

**移动端任务必须过此门。** 条件：
- 模拟器/真机已启动（iOS Simulator + Android Emulator）
- 修改前/后对比截图已附
- 多平台截图已附（iOS + Android，如涉及 Web 也覆盖）
- 暗色模式截图（如支持）
- Safe Area / 状态栏适配截图
- 无可见布局问题或 UI 异常
...
全部实现 Batch 完成
  → Gate C1.5 视觉验证通过
  → 步骤 1：spawn react-native-test-expert（单元/组件测试：Jest + RNTL）
  → 步骤 2：Web 端浏览器测试（spawn browser-test-expert，如有 Web/RNW 变更）
...
- Web 端：agent-browser 浏览器自动化（如有 react-native-web）
- E2E：Detox（iOS/Android 真机或模拟器）或 Maestro

**测试失败回退**：
1. 任一 agent 测试失败 → 分析失败报告，定位需修复的实现 Agent
2. spawn 原 RN 实现 Agent 执行修复（传递测试失败报告），修复后重新跑对应测试
...
├── spawn react-native-review-expert（RN 代码审查：组件架构/UI样式/状态管理/原生桥接/性能）
├── spawn security-review-expert（安全审计：OWASP Mobile Top 10/CVE/密钥检测）
└── spawn perf-review-expert（性能审计：启动时间/首屏渲染/内存/Bridge通信/包体积）
...
3. RN 审查不通过 → spawn 原 RN 实现 Agent（react-native-dev-expert / react-native-ui-expert / react-native-state-expert）
...
- iOS：`npx react-native run-ios --configuration Release` + TestFlight + App Store 提交
- Android：`cd android && ./gradlew bundleRelease` + Google Play 提交
- Fastlane 自动化分发（如配置）
- CodePush / OTA 更新配置（紧急修复无需重新提交应用商店）
...
向用户确认已进入 React Native 开发生命周期模式。

---

## 红线
- 原生桥接必须双端验证——iOS 和 Android 行为可能不同
- Hermes 引擎兼容性必须测试——部分 JSC 特性在 Hermes 上不可用
- 热更新不能包含原生代码变更——原生变更必须走应用商店
- 动画必须使用原生驱动——JS 驱动的动画性能不可接受
