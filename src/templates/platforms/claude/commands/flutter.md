---
description: Flutter 开发生命周期——需求→任务→计划→实现→质量→测试→评审→发布完整链路
name: flutter
model: inherit
argument-hint: "[Flutter需求描述]"
...
| **技术栈** | Dart + Flutter Widget 跨端移动端开发 |
| **Gate 序列** | A → B-DDD → B-BDD → B-TDD → B1 → C → C-impl → C1 → C1.5 → C2 → D → E（12 道闸门） |
| **强制 Gate C1.5** | 所有 Flutter UI 变更必须过视觉验证（Emulator 截图 + 多设备尺寸） |
...
| UI/布局/样式 | `flutter-ui-expert` |
| 状态/数据/路由 | `flutter-state-expert` |
| 测试 | `flutter-test-expert` |
| 审查 | `flutter-review-expert` |
...
Gate C-impl:
  Batch 1: [flutter-ui-expert, flutter-state-expert]   ← UI + 状态可并行
  Batch 2: [flutter-dev-expert]                          ← 集成组装
  Batch 3: [flutter-test-expert]                         ← 单元/组件测试
```
...
**关键差异**：Gate C1.5（视觉验证）强制不可跳过——所有 Flutter UI 变更必须提供 Emulator 截图证据。
...
- **Gate A 通过后可并行探索**（spawn 前 `gate_check("read")`）：
  ...
  - `external-resource-expert` × N（Flutter/Dart 最新文档与最佳实践并行搜索）
...
4. 每个任务 → `Agent()` 调用，选择 Flutter 代理路由表中的 `subagent_type`
5. **同 Batch 任务在同一条消息中批量发出**（不可逐个串行）
6. 等待整批完成，检查 plan patch

**Batch 执行顺序**：
- Batch 1: `flutter-ui-expert` + `flutter-state-expert` 并行
- Batch 2: `flutter-dev-expert` 集成组装
- Batch 3: `flutter-test-expert` 单元/组件测试
...
**Flutter 移动端任务必须过此门。** 条件：
- Flutter Emulator 已启动
- 修改前/后对比截图已附
- 多设备尺寸截图已附（phone 375x812 / tablet 768x1024）
- 关键 Widget 布局和样式已验证
- 无可见布局问题（溢出/重叠/错位）
...
全部实现 Batch 完成
  → 先过 Gate C1.5（视觉验证）
  → spawn flutter-test-expert（spawn 前 gate_check("spawn_test")）
  → 执行 flutter test + flutter drive
...
├── spawn flutter-review-expert（Flutter 代码审查：Widget/状态/性能/可访问性）
├── spawn mobile-architect（架构一致性验证）
└── spawn security-review-expert（安全审计：依赖 CVE/凭证存储/安全传输）
...
1. 加载 `Skill("code-quality-gate")`，重跑 `flutter analyze` + `flutter build`
2. 重跑测试套件（`flutter test` + `flutter drive`），确保无回归
...
- 加载 `Skill("shipping-and-launch")` 执行上线检查清单
...
2. 按测试类型并行 spawn flutter-test-expert 分工
...
2. 按审查领域并行：flutter-review-expert + mobile-architect + security-review-expert
3. 每个审查者独立评审，产出分级报告
...
| Gate C2 | `flutter-test-expert` 执行单元+E2E 测试 |
| Gate D | `flutter-review-expert` + `mobile-architect` + `security-review-expert` 并行；完成后 `qa-review-expert` 综合签核 |
...
## 红线
- Flutter 任务必须过视觉验证——Gate C1.5 不可跳过
- Widget 状态管理必须有对应测试——Gate C2 覆盖率门禁
- UI Widget 必须考虑多设备适配——响应式布局是强制要求
- 不修改后端 API 契约——Flutter 层只管界面和状态，后端接口不可动
- 不修改平台原生代码（Android/iOS 桥接层）除非任务明确要求
