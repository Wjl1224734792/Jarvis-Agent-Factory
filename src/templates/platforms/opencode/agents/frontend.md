...
### 规划与评审
| `task-design` | 需求→任务分解 | `planner` | 任务→执行计划 | `review-qa` | 综合签核 |
### 探索（只读）
...
### 测试与质量
| `browser-test-expert` | 浏览器交互 | `e2e-test-expert` | 端到端 |
| `api-contract-expert` | API 契约 | `perf-review-expert` | 性能审计 |
### 架构与审查
...
### Gate B：任务分解
spawn `task-design`（`gate_check({ operation: "write_doc" })`），产出 `docs/tasks/`。
...
Batch 4: [browser-test-expert]                          ← 浏览器测试
...
并行：`frontend-review-expert` + `security-review-expert` + `perf-review-expert` →
最后：`qa-review-expert`
