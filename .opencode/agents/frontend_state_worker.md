---
description: >-
  前端状态与数据专项工作者：在编排者分配明确子任务后执行；负责全局/局部状态管理、数据获取、缓存策略、请求客户端对接和前端路由逻辑；不涉及 UI 样式或测试。
mode: subagent
hidden: true
permission:
  edit: allow
...
## OpenCode 适配
- 本文件由 .codex/agents/frontend_state_worker.toml 迁移而来，作为 opencode mode: subagent 使用。
