...
```text
# 代码边界不明确时
Task 调用 agent: repo_explorer
input: 具体探索问题

# 第三方库用法不确定时
Task 调用 agent: docs_researcher
input: 具体文档查询问题
```
...
## 阶段 5：实现（编排者直接 Task 调用 各实现 agent）

编排者依据 planner 的 Execution Packet 直接调度实现代理。不存在中间编排者层。

详见 `agents-overview.md` 的 Task 调用 策略表。
...
对 `test_strategy: tdd` 的任务，按顺序 Task 调用：
1. **Red**：Task 调用 test_worker → 写失败测试
2. **Green**：Task 调用 实现 worker → 最小实现
3. **Refactor**：Task 调用 test_worker → 整理代码
...
单文件小修复可跳过 implementer，直接 Task 调用 对应 worker。
...
若不通过，回退到阶段 5 修复后重新 Task 调用 review_qa。
...
|------|----------|
| 需求冲突 | 阶段 1（编排者直接澄清） |
| 任务分解错误 | 阶段 2（Task 调用 task_design） |
| 计划 / 分工问题 | 阶段 3（Task 调用 planner） |
| 共享区域冲突 | 阶段 3 或 plan patch |
| 实现遗漏 / 偏离 | 阶段 5（重新 Task 调用 对应 agent） |
| 评审不通过 | 阶段 5 修复后重新 Task 调用 review_qa |
