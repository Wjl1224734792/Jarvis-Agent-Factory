...
4. 严格执行 Gate 闸门制度。每个 Gate 通过条件：
   ...
   - **Gate D**：实现文档、代码 diff、验证证据齐备、Gate C1/C2 报告已就绪
...
## 🟠 Gate C1：代码质量门（实现完成 → Lint + Type-check + Build）

Gate C1 是**实现完成后的第一道质量门**——所有实现 Batch 完成后，在进入测试验证（Gate C2）之前，必须通过 Lint、Type-check、Build 三项检查。

### Gate C1 通过条件

必须**全部**满足才能进入 Gate C2：

- [ ] **Lint 零错误**：代码风格检查通过，无 lint error（warning 可接受但需记录）
- [ ] **Type-check 零错误**：静态类型检查通过（TypeScript `tsc --noEmit`、Python `mypy`、Kotlin 编译检查等）
- [ ] **Build 成功**：项目构建成功（`npm run build` / `cargo build` / `go build` 等）
...
全部实现 Batch 完成
  │
  ├── 步骤 1：执行 Lint
  │     命令示例：npm run lint / eslint / ruff / golangci-lint
  │     判定：0 error → 通过 / 有 error → 回退对应实现 agent 修复
  │
  ├── 步骤 2：执行 Type-check
  │     命令示例：tsc --noEmit / mypy / cargo check
  │     判定：0 error → 通过 / 有 error → 回退对应实现 agent 修复
  │
  ├── 步骤 3：执行 Build
  │     命令示例：npm run build / cargo build / go build ./...
  │     判定：exit 0 → 通过 / exit ≠ 0 → 回退对应实现 agent 修复
  │
  └── 全部通过 → Gate C1 通过 → 进入 Gate C2
```

### 按项目类型自动选择检查命令

| 项目类型 | Lint | Type-check | Build |
|---------|------|------------|-------|
| **TypeScript/JavaScript** | `eslint` / `prettier --check` | `tsc --noEmit` | `npm run build` / `tsup` |
| **Python** | `ruff check` / `black --check` | `mypy` | — (解释型语言跳过) |
| **Rust** | `cargo clippy` / `rustfmt --check` | `cargo check` | `cargo build` |
| **Go** | `golangci-lint run` | `go vet` | `go build ./...` |
| **Kotlin/Android** | `ktlint` / `detekt` | 编译检查 | `./gradlew build` |
| **Swift/iOS** | `swiftlint` | 编译检查 | `xcodebuild` |
| **Flutter/Dart** | `flutter analyze` / `dart format --check` | `dart analyze` | `flutter build` |

### 失败处理

- **同一文件的 lint/type-check 错误** → 回退到该文件对应的实现 agent 修复，修复后仅重跑失败的检查项
- **Build 失败** → 回退到导致编译错误的实现 agent 修复
- **同一错误修复 2 次仍失败** → 标记该任务为 `BLOCKED`，阻止进入 Gate C2，但不影响其他已通过检查的任务
- **修复后** → 重新执行全部三项检查（Lint → Type-check → Build），确保修复未引入新问题
...
## 🟡 Gate C2：测试验证（代码质量通过 → 测试全部通过）

Gate C2 是**代码质量门与评审之间的测试验证门**。Gate C1 通过后方可进入此门。
