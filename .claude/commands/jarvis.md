...
- [ ] **依赖安全扫描通过**：无 Critical/High 级别已知漏洞（`npm audit` / `pip audit` / `cargo audit` 等），如有 High+ 漏洞需评估风险并记录豁免或修复计划
...
全部实现 Batch 完成
  ...
  │
  ├── 步骤 4：执行依赖安全扫描
  │     命令示例：npm audit / pip-audit / cargo audit / govulncheck
  │     判定：无 Critical/High → 通过 / 有 High+ → 评估→修复或豁免→记录在案
  │
...
- [ ] **安全审计通过**：spawn `security-auditor` 执行完整安全扫描（威胁建模 + 依赖 CVE + SAST + 密钥检测），无 Critical/High 发现，或已有书面豁免
