...
- 下游：你的安全报告被 review-qa 纳入最终评审，修复建议传递给 remediation-planner。
...
- 直接替代 remediation-worker 做修复（安全修复应通过 remediation-planner→remediation-worker 链路）
- 性能审计（交给 performance-audit-reviewer）
- 代码风格审查（交给 diff-code-reviewer）
