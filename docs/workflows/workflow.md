...
本文档定义了本仓库的默认多步骤交付流程，并与仓库根目录 **`AGENTS.md`** 中的多智能体路由、**TDD 编排**、验证与评审预期对齐；若仅有措辞差异以 **`AGENTS.md`** 为准。
...
由**主会话**在对话中直接向用户追问并完成（勿委派子代理：子代理无法向用户追问）。适用于：
...
- 工作分类：
  ...
  - 测试密集型工作，
- 每个任务的验证预期，
- 可在此阶段为各任务**草拟**测试策略（`tdd` / `test_after` / `manual_only`），由 **`planner` 在阶段 3 落为正式字段**并与 `docs/tasks` 中的「TDD」等标注对齐。
...
- 风险点，
- 共享区域职责，
- **每条实现类任务的 `test_strategy`**（见下文「TDD 与 test_strategy」），并与 `docs/tasks` 等文档中的「TDD」表述一致。
...
- 每条待实现任务带 **`test_strategy: tdd | test_after | manual_only`** 及必要时的验收说明。
...
- 遗漏的边界情况，
- 未完成的假设或后续事项，
- **`test_strategy: tdd` 的变更**：是否具备 **Red → Green**（及需要的 **Refactor**）证据；若仅有最终实现、缺少失败用例阶段说明，应打回或要求补证据。
...
## TDD 与 test_strategy（与 AGENTS.md 对齐）
...
1. **Red**：新增或修改测试，使当前行为**明确失败**；运行对应测试命令并保留失败输出或日志说明。
...
若上述区域的任务为 **`test_strategy: tdd`**，同一文件上的测试与实现步骤须**单线程**收敛，避免多代理同时修改。
...
- 范围或目标混乱时回滚到**主会话澄清**（对话对齐需求，必要时更新需求文档），
...
- **`test_strategy: tdd` 的任务**：具备可核对的 Red/Green（及需要的 Refactor）说明或历史，
- 关键风险和假设已可见，
- 评审已通过或剩余差距已明确记录。
