...
- **命令**：`/vue [需求描述]`
- **类别**：框架开发
- **说明**：Vue 3 Web 应用完整开发生命周期，Vue 3 + Composition API + TypeScript，C1.5 视觉验证强制。

## 使用场景
| 场景 | 说明 |
|------|------|
| Vue 3 SPA 开发 | 从零构建 Vue 3 单页应用 |
| 现有 Vue 项目迭代 | 功能新增、Bug 修复、组件重构 |
| Vue 组件库开发 | 高度可复用的 Vue 3 组件 |
| Vue 性能优化 | 渲染优化、bundle 分析、Lighthouse |
| 前端发布准备 | 构建优化 + CDN 部署 |

## 关键 Agent
| Agent | 职责 |
|-------|------|
| vue-dev-expert | Vue 3 业务逻辑、架构实现 |
| vue-ui-expert | Vue SFC 组件、CSS/animation 设计 |
| vue-state-expert | Pinia/Vuex 状态管理 |
| vue-test-expert | Vitest + Vue Test Utils 组件测试 |
| vue-review-expert | 组件架构/UI/状态/性能评审 |
...
```mermaid
flowchart TD
    START([用户输入 /vue]) --> A[Gate A: 需求澄清<br/>探索→澄清→靶向探索]
    ...
    C2C --> D[Gate D: 评审]
    D --> D1[vue-review-expert<br/>组件架构/UI/状态/性能]
